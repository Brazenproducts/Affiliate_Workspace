...
def build_jk_hub():
  ...
  <tbody>
    ...
    bc = bartact_card(
        ...
        ["Custom-cut, not universal fit","600D Polyester (or 1000D Cordura nylon for Coyote/OD/ACU) with PU waterproof backing","Mil-spec MOLLE on every seat","SAB airbag-compliant seam construction","Cut and hand-sewn in the USA","Fits all JK years: 2007-2018"]
    )
...
def build_jk_year(year_key, year_range, airbag_note, prev_url, next_url):
  ...
  <a href="/jk-2013-2018-seat-covers.html" {"class='active'" if year_key=='jk-2013-2018' else ""}>2013&ndash;2018</a>
    ...
    bc = bartact_card(
        ...
        img,
        collection,
        ["Custom-cut, not universal","600D Polyester (or 1000D Cordura nylon for Coyote/OD/ACU) with PU waterproof backing","Mil-spec MOLLE on every seat","SAB airbag-compliant seam construction",f"Confirmed fit: {year_range} JK 2-door and JKU 4-door","Cut and hand-sewn in the USA"]
    )
    cards = "".join(product_card(p) for p in JK_PRODUCTS)
...
<div class="container">
  ...
  <div class="faq-item"><h3>Is Bartact worth the price over Amazon options?</h3><p>Bartact uses 600D Polyester (or 1000D Cordura nylon for Coyote/OD/ACU) with PU waterproof backing, laminated foam and scrim, UV protection in the fabric milling, and mil-spec MOLLE &mdash; all sewn in the USA. Amazon alternatives use lighter materials without MOLLE. If MOLLE and USA-made matter to you, Bartact is worth it. If you just need basic seat protection, the Amazon picks are solid.</p></div>
...
def build_jl_hub():
    ...
    bc = bartact_card(
        ...
        BARTACT_IMGS["jl"],
        collection,
        ["Custom-cut, not universal","600D Polyester (or 1000D Cordura nylon for Coyote/OD/ACU) with PU waterproof backing","Mil-spec MOLLE on every seat","SAB airbag-compliant seam construction","Fits 2018-2024 JL 2-door and JLU 4-door","Cut and hand-sewn in the USA"],
...
<div class="container">
  ...
  <div class="faq-item"><h3>What&rsquo;s different about Bartact vs Amazon for the JL?</h3><p>Bartact uses 600D Polyester (or 1000D Cordura nylon for Coyote/OD/ACU) with PU waterproof backing, laminated foam and scrim, UV protection in the fabric milling, and mil-spec MOLLE webbing &mdash; sewn in the USA. Amazon alternatives (Smittybilt, Aierxuan, GIANT PANDA, FH Group) use lighter neoprene or faux leather without MOLLE. Both protect your seats; Bartact adds MOLLE functionality and USA-made quality.</p></div>
...
def build_tj_hub():
    ...
    bc = bartact_card(
        ...
        BARTACT_IMGS["tj"],
        collection,
        ["Custom-cut for TJ and LJ (Unlimited)","600D Polyester (or 1000D Cordura nylon for Coyote/OD/ACU) with PU waterproof backing","Mil-spec MOLLE on every seat","SAB airbag-compliant seam construction","Fits all TJ/LJ years: 1997-2006","Cut and hand-sewn in the USA"]
    )
...
def build_index():
  ...
  <a href="/tj-seat-covers.html" style="display:block;border:2px solid #e0e0e0;border-radius:12px;padding:22px 16px;text-decoration:none;text-align:center;color:#222;transition:.15s" onmouseover="this.style.borderColor='#c8860a';this.style.background='#fff8f0'" onmouseout="this.style.borderColor='#e0e0e0';this.style.background=''">
    ...
    bc = bartact_card(
        ...
        BARTACT_IMGS["jk"],
        "https://bartact.com/collections/jeep-wrangler-jk-seat-covers",
        ["Custom-cut, not universal fit","600D Polyester (or 1000D Cordura nylon for Coyote/OD/ACU) with PU waterproof backing","Mil-spec MOLLE on every seat","SAB airbag-compliant seam construction","Made in the USA","Fits JK, JKU, JL, JLU, TJ, and LJ"]
    )
