...
async function main() {
  for (const id of [1287344773, 3948249481239]) {
    ...
    // Replace the old power top block using a regex that handles both quote styles
    const body = d.product.body_html.replace(
      /<div style=["']background:#1a1a1a;border-left:4px solid #b8001f[^>]*>[\s\S]*?JL.*?Automatic Power Top Roof[\s\S]*?<\/div>/,
      newPowerTopBlock
    );

    if (body === d.product.body_html) {
      console.log('⚠️  No match found for id', id, '— check manually');
      continue;
    }
